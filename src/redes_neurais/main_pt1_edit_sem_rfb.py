...
OUTPUTS_DIR = BASE_DIR / "outputs pt1 edit sem rfb"
...
def print_metric_summary(results_by_model, metric_names):
    for model_name, metrics_dict in results_by_model.items():
        ...
        for metric_name in metric_names:
            ...
            print(
                f"  -> {metric_name.ljust(15)} | Média: {summary['mean']:.4f} | "
                f"Desvio: {summary['std']:.4f} | Max: {summary['max']:.4f} | Min: {summary['min']:.4f}"
            )
...
def build_model(name, X, Y, cfg):
    if name == "Perceptron":
        return Perceptron(X, Y, cfg["learning_rate"], cfg["max_epochs"])
    if name == "Adaline":
        return Adaline(X, Y, cfg["learning_rate"], cfg["precision"], cfg["max_epochs"])
    if name == "MLP":
        return MultilayerPerceptron(
            cfg["topology"], X, Y, cfg["learning_rate"], cfg["max_epochs"], cfg["precision"]
        )
    raise ValueError(f"Modelo não suportado: {name}")


def evaluate_binary(name, cfg, Xtr, Ytr, Xte, Yte):
    model = build_model(name, Xtr, Ytr, cfg)
    model.fit()
    y_tr_p = decode_binary_outputs(model.decision_function(Xtr))
    y_te_p = decode_binary_outputs(model.decision_function(Xte))
...
def etapa_1_spiral():
    ...
    print("\n[Status] Realizando estudos de complexidade (somente MLP)...")
    for study_name, config in mlp_study.items():
        ...
        plot_confusion_matrix(
            ...
            ["-1", "+1"],
            f"Matriz MLP {study_name}",
            f"etapa1_mlp_{study_name}_matriz.png",
        )

    # --- MONTE CARLO (500 Rodadas) ---
    rodadas = 500
    metrics = ["accuracy", "sensitivity", "specificity", "precision", "f1_score"]
    mc_results = {model_name: {metric: {"values": [], "rounds": []} for metric in metrics} for model_name in configs}

    print(f"\n[Status] Iniciando Monte Carlo ({rodadas} rodadas)...")
    for rodada in range(rodadas):
        print(f"  -> Rodada {rodada + 1}/{rodadas}", end="\r")
        Xtr, Ytr, Xte, Yte, _, _ = train_test_split_columns(X, Y, rng=rng)
        Xtr, Xte = normalize_bipolar(Xtr, Xte)

        for model_name, cfg in configs.items():
            res = evaluate_binary(model_name, cfg, Xtr, Ytr, Xte, Yte)
            for metric in metrics:
                mc_results[model_name][metric]["values"].append(res["metrics"][metric])
                mc_results[model_name][metric]["rounds"].append(
                    {
    ...
    for model_name in mc_results:
        for metric in metrics:
            mc_results[model_name][metric]["summary"] = summarize(mc_results[model_name][metric]["values"])
    print_metric_summary(mc_results, metrics)

    # 2. Plota os Boxplots para cada métrica
    for metric in metrics:
        plot_metric_boxplot(mc_results, metric, f"etapa1_boxplot_{metric}.png")

    # 3. Salva a melhor e a pior rodada (baseada na Acurácia) de cada modelo
    for model_name in configs:
        rounds_data = mc_results[model_name]["accuracy"]["rounds"]
        ...
        plot_confusion_matrix(
            ...
            ["-1", "+1"],
            f"{model_name} - Maior Acurácia",
            f"etapa1_{model_name.lower()}_melhor_matriz.png",
        )
        plot_learning_curve(
            best_round["history"],
            f"{model_name} - Maior Acurácia",
            f"etapa1_{model_name.lower()}_melhor_curva.png",
        )

        plot_confusion_matrix(
            worst_round["confusion_matrix"],
            ["-1", "+1"],
            f"{model_name} - Menor Acurácia",
            f"etapa1_{model_name.lower()}_pior_matriz.png",
        )
        plot_learning_curve(
            worst_round["history"],
            f"{model_name} - Menor Acurácia",
            f"etapa1_{model_name.lower()}_pior_curva.png",
        )
...
if __name__ == "__main__":
    ...
    etapa_1_spiral()
    print("\nProcessamento da Etapa 1 concluído. Verifique a pasta 'outputs pt1 edit sem rfb'.")
